...
public class ConcesionarioControlador {
    ...
    public void anadirCoche() {
        ...
        // Pedimos los datos del coche a la vista
        CocheDTO nuevo = vista.leerCoche();

        if (nuevo.getKilometros() < 0) {
            vista.mensajeErr("Los kilómetros no pueden ser negativos.");
            return;
        }

        if (nuevo.getPrecio() <= 0) {
            vista.mensajeErr("El precio debe ser mayor que 0.");
            return;
        }

        if (nuevo.getModelo() == null || nuevo.getModelo().trim().isEmpty()) {
            vista.mensajeErr("El modelo no puede estar vacío.");
            return;
        }

        if (nuevo.getAnio() <= 0) {
            vista.mensajeErr("El año no puede ser menor que 0.");
            return;
        }

        if (nuevo.getMarca() == null || nuevo.getMarca().trim().isEmpty()) {
            vista.mensajeErr("La marca no puede estar vacía.");
            return;
        }

        if (nuevo.getMatricula() == null || nuevo.getMatricula().trim().isEmpty()) {
            // Si la matrícula está vacía o solo tiene espacios, se muestra error
            vista.mensajeErr("La matrícula es obligatoria. No se puede dejar vacía.");
            return;
        }

        // Comprobamos si ya existe un coche con esa matrícula
        for (CocheDTO coche : coches) {
            if (coche.getMatricula().equalsIgnoreCase(nuevo.getMatricula())) {
                // Si la matrícula ya existe, mostramos error
                vista.mensajeErr("Ya existe un coche con esa matrícula.");
                return; // no se añade el coche
            }
        }

        // Si lo anterior esta correcto lo añade con el mensaje
        coches.add(nuevo);
        vista.mensaje("Coche añadido correctamente.");
    }
    ...
    public void registrarCliente() {
        ...
        if (nuevo.getDni() == null || nuevo.getDni().trim().isEmpty()) {
            vista.mensajeErr("El DNI es obligatorio.");
            return;
        }

        if (nuevo.getNombre() == null || nuevo.getNombre().trim().isEmpty()) {
            vista.mensajeErr("El nombre es obligatorio.");
            return;
        }

        if (nuevo.getTelefono() == null || nuevo.getTelefono().trim().isEmpty()) {
            vista.mensajeErr("El teléfono es obligatorio.");
            return;
        }
    ...
    public void registrarVenta() {
        ...
        // Comprobamos que el precio no sea negativo ni cero
        if (precio < 0) {
            vista.mensajeErr("El precio de venta debe ser mayor que 0.");
            return;
        }
